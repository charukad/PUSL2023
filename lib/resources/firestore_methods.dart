...
import 'package:mobilenew/models/post.dart';
...
class FireStoreMethods {
  ...
  Future<String> postChatMessage(
    ...
    try {
      if (text.isNotEmpty) {
        String messageId =
            const Uuid().v1(); // Generates a unique ID for the message
        // Posting message to 'groupChat' collection
        await _firestore.collection('groupChat').doc(messageId).set({
          'uid': uid,
          'name': name,
          'profilePic': profilePic,
          'text': text,
          'messageId': messageId,
          'datePublished': DateTime.now(), // Timestamp for the message
        });
        res = 'success';
      } else {
  ...
  Future<String> postEvent(
    ...
    String res = "Some error occurred";
    try {
      String eventId = const Uuid().v1(); // Creates a unique ID for the event
      Event event = Event(
        id: eventId,
        title: title,
        date: date,
